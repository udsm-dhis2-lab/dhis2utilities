...
function storeDataSets(headers) {
    ...
    promise.then(function(formArr) {
        localStorage.setItem('indicatorsData',JSON.stringify(formArr));
    });
}
...
function retrieveDataSetsObjectsAndStoreBackup() {
    ...
    JSON.parse(arr).forEach(function (dataSet) {
        var content = dataSet.dataEntryForm.htmlCode;
...
function readAndFormatHtmls(fileNamesArr) {
    for (var key in fileNamesArr) {
        fs.readFile(fileNamesArr[key], 'utf8', formatHtml)
    }
}

// fs.readFile('htmls/original.html', 'utf8', formatHtml);

function formatHtml(err, data) {
...
function readAndFormat() {
    ...
    var fileNamesArr = retrieveDataSetsObjectsAndStoreBackup();

    // read and For
    readAndFormatHtmls(fileNamesArr);
}
